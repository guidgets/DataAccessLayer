...
namespace DataAccessLayer.Wrappers
{
    ...
    public class TypePropertyWrapper : ComplexPropertyWrapper
    {
        public bool Add(Type type, string propertyName)
        {
            ...
            int index = this.GetWrapperIndex(cw.ClassType);

            if (index == -1)
                this.propertyWrappers.Add(new Pair(type, spw));
            else
                this.propertyWrappers.Insert(index, new Pair(type, spw));
        ...
        public override Result GetValue(object data, params Object[] parameters)
        {
            ...
            IPropertyWrapper wrapper = GetWrapper(data.GetType());
        ...
        {
           return GetWrapper(data.GetType()).GetValueUnsafe(data);
        }
        ...
        protected IPropertyWrapper GetWrapper(Type type)
        {
            foreach (Pair pair in this.propertyWrappers)
                if (pair.Wrapper.CanWorkWithType(type))
                    return pair.Wrapper;

            return null;
        }

        protected int GetWrapperIndex(Type type)
        {
            for (int i = 0; i < this.propertyWrappers.Count; i++)
            {
                if (this.propertyWrappers[i].Wrapper.CanWorkWithType(type))
                    return i;
            }
